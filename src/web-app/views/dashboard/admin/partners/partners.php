        ...
        <div class="bento-box glassy dashboard-box">
            <div class="dashboard">
                ...
                <div class="btn-wrapper inline">
                    <a class="btn outline" id="cancel-btn" href="/dashboard/partners/create">Nouveau</a>
                </div>
                <div class="result-table">
                    <?php
                        ...
                        $stmtCount = $connection->getConnection()->prepare("SELECT COUNT(*) AS CT FROM partner WHERE namePartner LIKE ?;");
                        ...
                        $stmt = $connection->getConnection()->prepare("SELECT * FROM partner WHERE namePartner LIKE ? LIMIT $DISPLAY_COUNT_PER_PAGE OFFSET ? ;");
                        $page = 1;
                        if(isset($_GET["page"])) {
                            $page = $_GET["page"];
                        }
                        ...
                        if($page<1) {
                            echo "<script>window.location.href='/dashboard/partners/?page=1';</script>";
                        }
                        else if($page>$TOTAL_PAGE_CT) {
                            echo "<script>window.location.href='/dashboard/partners/?page=$TOTAL_PAGE_CT';</script>";
                        }
                        ...
                        foreach($stmt->fetchAll() as $res) {
                            $template = new Template("partner.template.php");
                            $template->fill_placeholder("partner_logo_blob", base64_encode($res["logoPartner"]));
                            $template->fill_placeholder("partner_name", $res["namePartner"]);
                            $template->fill_placeholder("partner_id",  $res["idPartner"]);
                            $template->display();
                        }
                    ?>
