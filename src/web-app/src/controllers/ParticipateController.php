...
function handleParticipate() {
    ...
    switch($_POST["type"]) {
        case 'training':
            $stmt = $con->prepare("SELECT idTraining,
    ...
    switch($_POST["action"]) {
        case 'stop_participate':
            switch($_POST["type"]) {
                case 'training':
                    $stmt = $con->prepare("SELECT User_idUser, Training_idTraining
                    ...
                    $stmt = $con->prepare("DELETE FROM user_has_training WHERE User_idUser=? AND Training_idTraining=?");
                ...
                case 'competition':
                    ...
                    $stmt = $con->prepare("DELETE FROM user_has_competition WHERE User_idUser=? AND Competition_idCompetition=?");
                    $stmt->bindValue(1, $_SESSION['userConnect']);
                    $stmt->bindValue(2, $_POST["id"]);
                    $stmt->execute();
                    back_redirect(); 
        ...
        case 'participate':
            ...
            switch($_POST["type"]) {
                case 'training':
                    $stmt = $con->prepare("INSERT INTO user_has_training (User_idUser, Training_idTraining) VALUES (?,?)");
                    $stmt->bindValue(1, $_SESSION['userConnect']);
                    $stmt->bindValue(2, $_POST["id"]);
                    $stmt->execute();
                    back_redirect(); 
                    return;
                case 'competition':
                    $stmt = $con->prepare("INSERT INTO user_has_competition (User_idUser, Competition_idCompetition) VALUES (?,?)");
                    $stmt->bindValue(1, $_SESSION['userConnect']);
                    $stmt->bindValue(2, $_POST["id"]);
                    $stmt->execute();
                    back_redirect(); 
